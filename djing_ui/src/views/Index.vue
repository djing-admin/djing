...
const { fetchCards } = useFetchCards(resource_name);

const { fetchLenses } = useFetchLenses(resource_name);

const { fetchFilters } = useFetchFilters(resource_name);
...
const {
  ...
  current_page,
  current_per_page,
} = useFetchResources(resource_name);

const { fetchActions } = useFetchActions(resource_name);
...
const should_show_dropdown = computed(() => {
  return resourceIndexStore.data.actions.length > 0;
});
...
<template>
  <LoadingView :loading="resourceIndexStore.loading">
    <Head :title="resourceInformation.label" />
    ...
    >
      <span v-html="resourceInformation.label" />
    </Heading>
