...
const { card, index, resource_name, resource_id } = defineProps({
  card: {
    type: Object,
    required: true,
  },
...
const metric_endpoint = computed(() => {
  if (resource_name && resource_id) {
    return `/djing-api/${resource_name}/${resource_id}/metrics/${card.uri_key}`;
  }

  if (resource_name) {
    return `/djing-api/${resource_name}/metrics/${card.uri_key}`;
  }
